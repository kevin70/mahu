...
export const MartProductEdit = () => {
  ...
  const basicPanel = (
    <Card title="产品信息">
      <ProFormText label={'SPU'} name={'id'} disabled />
              ...
              render={(_row, _record, rowIdx) => {
                return (
                  <>
                    {rowIdx + 1}
                    <div style={{ display: 'none' }}>
                      <FormItem noStyle name={[rowIdx, 'id']}>
                        <Input />
                      </FormItem>
                    </div>
                  </>
                ...
                <Table.Column
                  ...
                  render={(_value, _record, rowIdx) => {
                    return (
                      <>
                        <div style={{ display: 'none' }}>
                          <FormItem noStyle name={[rowIdx, 'attributes', attributeIdx, 'id']}>
                            <Input />
                          </FormItem>
                        </div>
                        ...
                        <AttributeFormItem
                          onLoaded={(data) => {
                            form.setFieldValue(
                              ['variants', rowIdx, 'attributes', attributeIdx, 'attributeId'],
                              data.id
                            );
                            form.setFieldValue(
      ...
      <ProForm
        ...
        request={async (params) => {
          ...
          for (const attr of product.attributes || []) {
            attributeList.push(attr.attributeId);
          }

          // 整理变体属性
          const firstVariant = product.variants?.find((_, i) => i === 0);
          for (const attr of firstVariant?.attributes || []) {
            if (attr.attributeId && variantAttributeList.getIndex(attr.attributeId) === -1) {
              variantAttributeList.push(attr.attributeId);
            }
          }
