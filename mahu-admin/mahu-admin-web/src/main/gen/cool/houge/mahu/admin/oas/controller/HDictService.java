...
public interface HDictService extends HttpService {
    ...
    default void routing(HttpRules rules) {
        rules.post("/sys/dicts/{type_code}", authenticate().andAuthorize("SYS_DICT:W"), this::addSysDictData);
        rules.post("/sys/dicts", authenticate().andAuthorize("SYS_DICT:W"), this::createSysDictType);
        rules.delete("/sys/dicts/{type_code}", authenticate().andAuthorize("SYS_DICT:W"), this::deleteSysDictType);
        rules.get("/sys/dicts/{type_code}", authenticate().andAuthorize("SYS_DICT:R"), this::getSysDictType);
        rules.get("/sys/dicts", authenticate().andAuthorize("SYS_DICT:R"), this::pageSysDictType);
        rules.put("/sys/dicts/{type_code}", authenticate().andAuthorize("SYS_DICT:W"), this::updateSysDictType);
    }

    ///
    /// `POST /sys/dicts/{type_code}` 新增字典数据
    ///
    ...
    ///
    /// `DELETE /sys/dicts/{type_code}` 删除字典类型
    ///
    /// @param request the server request
    /// @param response the server response
    ...
    ///
    /// `GET /sys/dicts/{type_code}` 获取字典类型数据
    ///
    /// @param request the server request
    /// @param response the server response
    ...
    ///
    /// `PUT /sys/dicts/{type_code}` 修改字典类型
    ///
    /// @param request the server request
    /// @param response the server response
