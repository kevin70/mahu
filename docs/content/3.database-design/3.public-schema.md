---
title: public 模式表结构
description: 介绍 Mahu 数据库 public 模式中的公共表结构及字典数据设计。
---
...
| 字段名 | 数据类型 | 约束 | 说明 |
|--------|----------|------|------|
| id | VARCHAR(50) | PRIMARY KEY | 字典分组 ID |
| name | VARCHAR(255) | | 字典分组名称 |
...
| type_id | VARCHAR(50) | NOT NULL | 字典分组代码 |
| created_at | TIMESTAMP | | 创建时间 |
| updated_at | TIMESTAMP | | 更新时间 |
...
-- `type_id`：外键，关联到 `dict_group` 表的 `id` 字段

## 表关系设计

### 字典分组与字典数据关系
- `dict_group` 表与 `dict` 表是一对多关系
- 一个字典分组可以包含多个字典数据项
- `dict.type_id` 字段引用 `dict_group.id` 字段
- 通过 `dict_type_id_ui` 唯一索引确保每个字典分组在 `dict` 表中只有一条记录（根据业务需求）

### 数据完整性约束
1. **外键约束**：`dict.type_id` 必须引用 `dict_group.id` 中存在的值
2. **唯一性约束**：`dict_group.id` 是主键，确保字典分组ID唯一
