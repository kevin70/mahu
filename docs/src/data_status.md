...
| 状态值 | 英文名称     | 中文名称 | 说明                      |
|-----|----------|------|-------------------------|
| 10  | DRAFT    | 草稿   | 未提交的临时记录                |
| 11  | PENDING  | 待审核  | 未验证                     |
| 12  | ACTIVE   | 活跃   | 正常                      |
| 89  | REJECTED | 被拒绝  | 未审核未通过的记录               |
| 90  | INACTIVE | 禁用   | 临时性禁用，可通过操作重新激活 (如解冻账号) |
| 99  | BLOCKED  | 禁用   | 管理员停用                   |
...
| 状态值 | 英文名称            | 中文名称 | 说明                       |
|-----|-----------------|------|--------------------------|
| 0   | DRAFT           | 草稿   | 未提交的临时订单（B2B常见）          |
| 1   | PENDING_PAYMENT | 待支付  | 订单已创建等待支付（含未支付倒计时）       |
| 2   | PAID            | 已支付  | 支付成功待处理                  |
| 3   | PROCESSING      | 处理中  | 仓库拣货/服务准备阶段              |
| 4   | SHIPPED         | 已发货  | 物流已发出（含物流单号）             |
| 5   | DELIVERED       | 已送达  | 物流显示签收（可能未实际收货）          |
| 6   | COMPLETED       | 已完成  | 客户确认收货/服务完成且无争议          |
| 7   | CANCELLED       | 已取消  | 用户或系统取消（需区分为支付前取消/支付后取消） |
| 8   | REFUNDED        | 已退款  | 全额退款完成                   |
| 9   | PARTIAL_REFUND  | 部分退款 | 部分商品/金额退款                |
| 10  | FAILED          | 失败   | 支付失败/库存不足等               |
